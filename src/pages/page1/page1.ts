...
import { GoogleMaps, GoogleMap, Marker, LatLng, Polyline, LatLngBounds } from "@ionic-native/google-maps";
@Component({
	selector: 'page-page1',
...
export class Page1 {
	...
	setMarker(data){
		...
		if (!this.marker) {
			...
			if (this.follow)
				this.map.animateCamera({ tilt: 45, zoom: 17, duration: 1000, target: this.position, bearing: data.heading });
			else{
				var latlngs = new LatLngBounds(this.points);
				this.map.animateCamera({target: latlngs,duration: 1000})	
			}
		}
	}
	...
	updatePolyLine(data){
		if(!this.route){
			...
			this.route.setPoints(this.points);
		}
	}
